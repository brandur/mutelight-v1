...
def route_path(item)
  ...
  # Use permalink attribute if possible
  return item[:permalink] if item[:permalink]

  # All logic below this line must be verified for my naming conventions
  # -- Brandur (2010/07/14)
...
def derive_created_at(item)
  parts = item.identifier.gsub('-', '/').split('/')[2,3]
  date = '1980/1/1'
  begin
